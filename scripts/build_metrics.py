...
res_times = [hours_between(i.get("createdAt"), i.get("closedAt")) for i in closed if i.get("closedAt")]
res_times = [r for r in res_times if r is not None]

now = datetime.now(timezone.utc)
overdue = [i for i in open_issues if (d := parse_date(i.get("body", ""), "planned_due")) and d < now]
...
metrics = {
    ...
    "average_resolution_hours": round(sum(res_times) / len(res_times), 2) if res_times else 0,
